...
df_all = pd.read_csv(HERE / "results" / "macrobenchmark_countvectorizer.csv")
...
col_vals = sorted(df["n_unique_words"].unique())
row_vals = [1_000_000] if only_1m else [10_000, 100_000, 1_000_000]
...
for i, dataset_length in enumerate(row_vals):
    ...
    sub_sklearn = df_sklearn[df_sklearn["dataset_length"] == dataset_length]
    x = np.arange(len(col_vals))

    sklearn_fit, sklearn_transform = [], []
    for c in col_vals:
        row = sub_sklearn[sub_sklearn.n_unique_words == c]
    ...
    for offset, n_jobs in zip(offsets[1:], n_jobs_vals):
        ...
        for c in col_vals:
            row = sub[(sub.n_unique_words == c) & (sub.n_jobs == n_jobs)]
        ...
        if n_jobs in (1, 8):
            origin_nudge = {1: -1, 8: 0}[n_jobs] * 0.015
...
fig.savefig(
    HERE / "plots" / f"macrobenchmark_countvectorizer_njobs_{out_suffix}.png",
    dpi=150,
    bbox_inches="tight",
)
plt.show()
