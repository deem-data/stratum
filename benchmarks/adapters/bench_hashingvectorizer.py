"""End-to-end benchmark of Stratum's Rust-backed HashingVectorizer.
...
pandas-to-list materialization, adapter validation, ASCII scanning, FFI, native
hashing, NumPy handoff, and SciPy CSR wrapping.
...
* memory reports the peak of one unprofiled transform after resetting Linux VmHWM;
* profiling runs one separate Rust transform with ``debug_timing`` enabled.
"""
...
_STAGE_SKIP = frozenset({"hv transform"})
...
def _new_vectorizer(HashingVectorizer, version: str):
...
def _assert_expected_backend(vectorizer, version: str) -> None:
    rust_enabled = vectorizer._rust_enabled()
    if version == "rust" and not rust_enabled:
        raise RuntimeError(
            "The Rust benchmark unexpectedly disabled the Rust backend"
        )
    if version == "sklearn" and rust_enabled:
        raise RuntimeError("The sklearn benchmark unexpectedly enabled Rust")


def _warm_up(HashingVectorizer, X, version: str) -> None:
    """Warm the selected backend without retaining the output."""
    X_small = X[: min(1000, len(X))]
    warm_vectorizer = _new_vectorizer(HashingVectorizer, version)
    warm_result = warm_vectorizer.transform(X_small)
...
def _output_signature(matrix) -> dict:
    import numpy as np
...
def _run_timing_worker(
    ...
    for repeat in range(timing_repeats):
        gc.collect()
        vectorizer = _new_vectorizer(HashingVectorizer, version)
        ...
        if expected_shape is None:
            ...
            if shape != expected_shape or nnz != expected_nnz:
                raise RuntimeError(
                    f"{version} timing repetition {repeat + 1} produced a "
                    "different shape or nnz"
                )
        if repeat == timing_repeats - 1:
            ...
            output_signature = _output_signature(result)
...
def _run_memory_worker(HashingVectorizer, X, version: str) -> dict:
    vectorizer = _new_vectorizer(HashingVectorizer, version)
    # Exercise the procfs reset path before establishing the baseline so its
    # own one-time allocations cannot be attributed to transform.
    _reset_peak_rss()
    gc.collect()
...
def _run_profile_worker(HashingVectorizer, X, version: str) -> dict:
    if version != "rust":
...
def run_worker(
    ...
    _warm_up(HashingVectorizer, X, version)

    if worker_mode == "timing":
        payload = _run_timing_worker(
            HashingVectorizer, X, version, timing_repeats
        )
    elif worker_mode == "memory":
        payload = _run_memory_worker(HashingVectorizer, X, version)
    elif worker_mode == "profile":
        payload = _run_profile_worker(HashingVectorizer, X, version)
    else:
