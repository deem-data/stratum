...
class ParallelScheduler(Scheduler):
    ...
    def linearize_dag(self, dag_sink: Op):
        ...
        logger.debug(f"Parallel groups: \n\n{"\n\n".join(f"[{",".join(op.name for op in g)}]" for g in parallel_groups.values())}\n")
    ...
    def compute(self, start_pos: int, mode="fit_transform"):
        ...
        if mode == "predict":
            ...
            if isinstance(last_block, list):
                # For parallel groups, get output from the last op in the group
                pred = last_block[-1].intermediate
            else:
                pred = last_block.intermediate
    ...
    def process_block(self, block):
        ...
        if isinstance(block, list):
            ...
            if self.backend == "process" or (self.backend == "auto" and all(isinstance(op, EstimatorOp) for op in ops)):
                # Use process-based parallel processing with joblib (like ParallelBlockOp)
                ...
                # Use thread-based parallel processing for non-estimator ops
    ...
    def close(self):
        """Close the ThreadPoolExecutor and wait for all pending tasks to complete."""
